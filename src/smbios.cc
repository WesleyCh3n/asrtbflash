...
bool SMBIOS::ProcBIOSInfo(SMBIOS *T, void *p) {
  ...
  if (T->m_wszBIOSVendor) {
    ...
    ::MultiByteToWideChar(CP_ACP, NULL, Vendor, nVendor, T->m_wszBIOSVendor,
                          nVendor + 1);
  }
  if (T->m_wszBIOSVersion) {
    ::SecureZeroMemory(T->m_wszBIOSVersion, sizeof(WCHAR) * (nVersion + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Version, nVersion, T->m_wszBIOSVersion,
                          nVersion + 1);
  }
  if (T->m_wszBIOSReleaseDate) {
    ::SecureZeroMemory(T->m_wszBIOSReleaseDate, sizeof(WCHAR) * (nDate + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Date, nDate, T->m_wszBIOSReleaseDate,
                          nDate + 1);
  }
...
bool SMBIOS::ProcSysInfo(SMBIOS *T, void *p) {
  ...
  if (T->m_wszSysManufactor) {
    ::SecureZeroMemory(T->m_wszSysManufactor,
                       sizeof(WCHAR) * (nManufactor + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Manufactor, nManufactor,
                          T->m_wszSysManufactor, nManufactor + 1);
  }
  if (T->m_wszSysProductName) {
    ::SecureZeroMemory(T->m_wszSysProductName,
                       sizeof(WCHAR) * (nProductName + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, ProductName, nProductName,
                          T->m_wszSysProductName, nProductName + 1);
  }
  if (T->m_wszSysVersion) {
    ::SecureZeroMemory(T->m_wszSysVersion, sizeof(WCHAR) * (nVersion + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Version, nVersion, T->m_wszSysVersion,
                          nVersion + 1);
  }
  if (T->m_wszSysSerialNumber) {
    ::SecureZeroMemory(T->m_wszSysSerialNumber,
                       sizeof(WCHAR) * (nSerialNumber + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, SerialNumber, nSerialNumber,
                          T->m_wszSysSerialNumber, nSerialNumber + 1);
  }
  ...
  if (pSystem->Header.Length > 0x19) {
    ...
    if (T->m_wszSysFamily) {
      ...
      ::MultiByteToWideChar(CP_ACP, NULL, Family, nFamily, T->m_wszSysFamily,
                            nFamily + 1);
    }
  }
  return true;
...
bool SMBIOS::ProcBoardInfo(SMBIOS *T, void *p) {
  ...
  if (T->m_wszBoardManufactor) {
    ::SecureZeroMemory(T->m_wszBoardManufactor,
                       sizeof(WCHAR) * (nManufactor + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Manufactor, nManufactor,
                          T->m_wszBoardManufactor, nManufactor);
  }
  if (T->m_wszBoardProductName) {
    ::SecureZeroMemory(T->m_wszBoardProductName,
                       sizeof(WCHAR) * (nProductName + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, ProductName, nProductName,
                          T->m_wszBoardProductName, nProductName);
  }
  if (T->m_wszBoardVersion) {
    ::SecureZeroMemory(T->m_wszBoardVersion, sizeof(WCHAR) * (nVersion + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, Version, nVersion, T->m_wszBoardVersion,
                          nVersion);
  }
  if (T->m_wszBoardSerialNumber) {
    ::SecureZeroMemory(T->m_wszBoardSerialNumber,
                       sizeof(WCHAR) * (nSerialNumber + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, SerialNumber, nSerialNumber,
                          T->m_wszBoardSerialNumber, nSerialNumber);
  }
  if (T->m_wszBoardAssetTag) {
    ::SecureZeroMemory(T->m_wszBoardAssetTag, sizeof(WCHAR) * (nAssetTag + 1));
    ::MultiByteToWideChar(CP_ACP, NULL, AssetTag, nAssetTag,
                          T->m_wszBoardAssetTag, nAssetTag);
  }
  if (pBoard->Header.Length > 0x08) {
    ...
    if (T->m_wszBoardLocation) {
      ::SecureZeroMemory(T->m_wszBoardLocation,
                         sizeof(WCHAR) * (nLocation + 1));
      ::MultiByteToWideChar(CP_ACP, NULL, Location, nLocation,
                            T->m_wszBoardLocation, nLocation);
    }
  }
  return true;
}
...
void SMBIOS::ParseSMBIOSStruct(void *Addr, UINT Len) {
  ...
  for (;;) {
    ...
    while (0 != (*nt | *(nt + 1))) {
      nt++; // skip string area
      if (nt >= lastAddress)
        break; // avoid infinite loop when got bad structure of SMBIOS
    }
    nt += 2;
    if (nt >= lastAddress)
      break;
...
SMBIOS::SMBIOS()
    : m_BIOSSysVersion(0UL), m_BIOSECVersion(0UL), m_wszBIOSVendor(NULL),
      m_wszBIOSVersion(NULL), m_wszBIOSReleaseDate(NULL),
      m_wszSysManufactor(NULL), m_wszSysProductName(NULL),
      m_wszSysVersion(NULL), m_wszSysSerialNumber(NULL), m_SysUUID(GUID_NULL),
      m_wszSysSKU(NULL), m_wszSysFamily(NULL), m_wszBoardManufactor(NULL),
      m_wszBoardProductName(NULL), m_wszBoardVersion(NULL),
      m_wszBoardSerialNumber(NULL), m_wszBoardAssetTag(NULL),
...
void SMBIOS::initialization(void) {
  GET_SYSTEM_FIRMWARE_TABLE pGetSystemFirmwareTable =
      (GET_SYSTEM_FIRMWARE_TABLE)GetProcAddress(GetModuleHandle("kernel32"),
...
bool SMBIOS::getWmiSmbios(BYTE **data, UINT *length) {
  ...
  result = pLoc->ConnectServer(_bstr_t(L"ROOT\\CIMV2"), NULL, NULL, 0, NULL, 0,
                               0, &pSvc);
  if (!SUCCEEDED(result)) {
    pLoc->Release();
    CoUninitialize();
  ...
  result = CoSetProxyBlanket(pSvc, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL,
  ...
  result = pLoc->ConnectServer(_bstr_t(L"ROOT\\WMI"), NULL, NULL, 0, NULL, 0, 0,
                               &pSvcSmbios);
  if (!SUCCEEDED(result)) {
    pLoc->Release();
    CoUninitialize();
  ...
  result = CoSetProxyBlanket(pSvcSmbios, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE,
  ...
  result = pSvcSmbios->CreateInstanceEnum(
      (wchar_t *const)L"MSSMBios_RawSMBiosTables", 0, NULL, &pEnumerator);
  if (SUCCEEDED(result)) {
    while (pEnumerator) {
